...
The compression function applies a 7-round keyed permutation $v' = E(m, v)$ to the state $v_0, \dots, v_{15}$, keyed by the message $m_0, \dots, m_{15}$. This keyed permutation here is identical to the one underlying BLAKE2s, and is reproduced in Appendix~\ref{sec:roundfn}.
...
\begin{itemize}
  \item In the \textbf{hash} mode, $k_{0} \ldots k_{7}$ are the constants $\IV_0 \ldots \IV_7$, and no additional flags are set. 
  \item In the \textbf{keyed\_hash} mode, $k_{0} \ldots k_{7}$ are parsed in little-endian order from the 32-byte key given by the caller, and the \texttt{KEYED\_HASH} flag is set for every compression.
  \item  In the \textbf{derive\_key} mode, the key is again given by the caller, and the \texttt{DERIVE\_KEY} flag is set for every compression.
\end{itemize}
...
\caption{Throughput at different message lengths on Intel Kaby Lake with AVX2
    and 4 physical cores. This is a multi-threaded implementation of BLAKE3.}
...
\caption{Throughput at different message lengths on Intel Skylake-SP with
...
\caption{Throughput at different message lengths on ARM Cortex-M0.}
\label{fig:rpizero}
\end{figure}
...
BLAKE3 has explicit support for a key derivation mode, \textbf{derive\_key}. In
...
v1"}. If the input key material is not already 256 bits, it should be converted
to 256 bits first using the \textbf{hash} mode. The \textbf{derive\_key} mode
is intended to replace the BLAKE2 personalization parameter for its most common
use cases.
...
Like \textbf{keyed\_hash}, \textbf{derive\_key} does not require any extra
compressions. For context strings up to 64 bytes and derived key lengths up to
64 bytes, \textbf{derive\_key} is a single compression. For comparison, HKDF
