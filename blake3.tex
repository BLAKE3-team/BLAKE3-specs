...
The compression function takes five inputs. Each word is 32 bits:

\begin{itemize}
    \item The 32-byte chaining value, $h_{0} \ldots h_{7}$.
    \item The 64-byte message block, $m_{0} \ldots m_{15}$.
    \item A 64-bit offset, $t_{0}$ and $t_{1}$.
    \item The block length, $b$.
    \item A flags word, $f$.
\end{itemize}

The compression function uses a 16-word internal state $v_{0} \ldots v_{15}$.
It initializes the state as follows. Here $\oplus$ denotes bitwise
exclusive-or, and $\IV_{0} \ldots \IV_{7}$ are constants given in Appendix A:
...
t_{0} \oplus \IV_{4} & t_{1} \oplus \IV_{5} & b \oplus \IV_{6} & f \oplus \IV_{7}
\end{pmatrix}
\end{equation}

The compression function then applies the round function 7 times, each time
using the state $v_{0} \dots v_{15}$, the message block $m_{0} \ldots m_{15}$,
and a message schedule. The round function and the message schedules for each
round are the same as in BLAKE2s. They are reproduced in Appendix A.
