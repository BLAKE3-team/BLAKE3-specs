...
BLAKE3 uses 1 KiB chunks. The chunk size is a tradeoff between peak throughput
for long inputs, which benefits from larger chunks, and the parallelism degree
for medium-length inputs, which benefits from shorter chunks.

The benefit of a larger chunk size is that the tree contains fewer parent
nodes. The number of parent nodes is equal to the number of chunks minus one,
so doubling the chunk size cuts the number of parent nodes in half. However,
note that parent nodes in the same level of the tree can be compressed in
parallel, so having twice as many parent nodes does not necessarily mean that
the implementation spends twice as much time compressing them.

On the other hand, the benefit of a smaller chunk size is that the more chunks
there are, the more work the implementation can do in parallel. This is not a
concern for very long inputs, where all the SIMD lanes and possibly CPU cores
of the machine will be occupied regardless. But at medium input lengths, this
has a huge impact on performance. Consider that with 1 KiB chunks, an 8 KiB
input can occupy 8 SIMD lanes, which means 256-bit AVX2 vector instructions on
modern x86 platforms. But if we increased the chunk size to 2 KiB, the same
input could only occupy 4 SIMD lanes, and the high throughput of AVX2 would be
left on the table.

Thus we want to pick the smallest chunks size we can to maximize medium-length
parallelism, without incurring "excessive" overhead from parent nodes for long
inputs. In our measurements, 1 KiB chunks provide at least 90\% of the maximum
possible throughput (that is, the throughput with 32 KiB or 64 KiB chunks,
where parent node overhead is negligible), on both modern x86 platforms and on
the ARM1176.

There are other minor performance considerations to be aware of. A larger chunk
size slightly reduces the size of the CV stack as described in
\S\ref{sec:memory}. Also, verified streaming (see
\S\ref{sec:verifiedstreaming}) requires buffering chunks, and incremental
update (see \S\ref{sec:incrementalupdate}) requires re-hashing chunks, so both
of those use cases benefit from a small chunk size.

\subsection{Word Size}\label{sec:wordsize}

BLAKE3 uses 32-bit words. The performance tradeoffs between 32-bit and 64-bit
words are complex, and they depend on both the platorm and the size of the
input.

Many sources note that SHA-512 (because it uses 64-bit words) is faster on
64-bit machines than SHA-256 (because it uses 32-bit words). A similar effect
applies to BLAKE2b and BLAKE2s. However, this effect does not apply to
algorithms that incorporate more SIMD parallelism. BLAKE2sp is faster than
BLAKE2bp on modern x86 platforms. This is because the SIMD instructions that
operate on 8 lanes of 32-bit words have the same throughput as those that
operate on 4 lanes of 64-bit words. The remaining difference for long inputs is
that the smaller state size of BLAKE2sp requires fewer rounds of compression.

Instead, performance differences between the word sizes on x86 are restricted
to short inputs. A 32-bit compression function takes fewer cycles than a 64-bit
one, so inputs up to 64 bytes perform better with 32-bit words. On the other
hand, a 64-bit compression function has higher throughput prior to multi-chunk
parallelism kicking in, so inputs longer than 64 bytes but shorter than one
chunk perform better with 64-bit words. Neither of these effects is likely to
be noticeable in typical applications, which only bottleneck on hash function
throughput for very long inputs.

The decisive difference between the word sizes, then, is performance on smaller
architectures and embedded systems. BLAKE3 is designed to be a general-purpose
hash function, suitable for replacing both BLAKE2b and BLAKE2s. On platforms
like the ARM1176 where BLAKE2s does well, a 64-bit hash function could be a
performance regression. Choosing 32-bit words is a substantial benefit for
these platforms, with little or no downside for x86 implementations.
Cross-platform flexibility is also a benefit for protocol designers, who have
to consider the performance of an algorithm across many different platforms.
