...
There are two different approaches to using SIMD in a BLAKE3 implementation,
and both are important for high performance. The first approach is to use
128-bit vectors to represent the 4-word rows of the state matrix. The second
approach is to use vectors of any size to represent words in several different
states, which are compressed in parallel.

The first approach is similar to how SIMD is used in BLAKE2b or BLAKE2s, and it
is applicable to inputs of any length. The state $v_0 \ldots v_{15}$ is
arranged into four 128-bit vectors. The first vector contains the state words
$v_0 \ldots v_3$, the second vector contains the state words $v_4 \ldots v_7$,
and so on. Implementing the G function (Appendix~\ref{sec:roundfn}) with vector
...
mix diagonals. Finally the matrix is undiagonalized, to prepare for the column
...
$v_1$ word from each state, and so on, 16 vectors in total. The number of
chunks hashed determines the width of the vector, so for example 4 chunks would
occupy a 128-bit vector. Here the G function operates on one column or diagonal
at a time, but across all of the states. When enough input is available, this
approach is much more efficient than the first approach, because the
diagonalization and undiagonalization steps are not required. Given enough
input, this approach also scales to wider vector instructions like AVX2 and
AVX-512.
