...
\end{tikzpicture}
    \caption{
        Example tree structures, from 1 to 4 chunks. Note that blocks are
        always added along the right edge of the tree, meaning that any left
        subtree is stable as the tree grows.
    }
    \label{fig:fourchunks}
\end{figure}

The compression function is used to derive an 8-word chaining value from each
chunk and parent node. The chaining value of the root node, encoded as 32 bytes
in little-endian order, is the BLAKE3 hash of the input. BLAKE3 supports input
of any byte length $0 \leq \ell < 2^{64}$.
...
Efficient parallel chunk hashing.
...
HMAC requires three extra compressions. The \textbf{keyed\_hash} mode in
BLAKE3, however, requires no extra compressions.
...
most common use cases. Key derivation can encourage better security than
personalization, by cryptographically isolating different components of an
application from one another. This limits the damage if one component
accidentally leaks its key.

For \textbf{derive\_key}, the input should be a hardcoded, globally unique,
application-specific context string. A good default format for such strings is
"[application] [date] [purpose]", e.g. "example.com 2019-12-25 16:18:03 session
tokens v1".

Like \textbf{keyed\_hash}, \textbf{derive\_key} requires no extra compressions.
For context strings up to 64 bytes and derived key lengths up to 64 bytes,
\textbf{derive\_key} is a single compression.
