...
\begin{itemize}
    \item \textbf{A tree structure} with unbounded
        degree of parallelism. This is the largest performance improvement.
    \item \textbf{Fewwer rounds} in the compression function  This is
    another performance improvement.
    \item \textbf{Three modes}: hash, keyed hash, and
    key derivation.  This simple API replaces the BLAKE2 parameter block.
%    \flag{hash(input)},
 %       \flag{keyed_hash(key, input)}, and
  ...
  %      \linebreak
   %     \flag{derive_key(context, key_material)}. 
    ...
    \item \textbf{Built-in extendable output}, parallelizable and
    seekable (like BLAKE2X, but unlike SHA\=/3 or HKDF).
\end{itemize}
