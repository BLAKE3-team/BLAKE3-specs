...
key. And the \flag{derive_key} mode takes a context string and key material,
both of any length, and returns a derived key. As described in
\S\ref{sec:extendable}, all three modes can return output of any length. The
intended usage of \flag{derive_key} is given in \S\ref{sec:kdf}.
...
k_7$ are parsed in little-endian order from the 32-byte key given by the
