...
\begin{center}
  \begin{minipage}{0.92\linewidth}
      ...
      improved version of BLAKE2 and its predecessor, the SHA\=/3 finalist
      ...
      BLAKE2b/s. At the same time, BLAKE3 is also faster than SHA\=/2, SHA\=/3,
      and BLAKE2 on smaller architectures and microcontrollers. BLAKE3 is a
...
large part because of its performance in software. BLAKE2b and BLAKE2s are
included in the Python and Go standard libraries, and BLAKE2b is included in
GNU Coreutils as the \texttt{b2sum} utility. BLAKE2b is also Libsodium's
\texttt{generic\_hash} and the basis for Argon2, the winner of the Password
Hashing Competition.

However, one of the drawbacks of BLAKE2 has been its large number of variants.
The original BLAKE2 paper described 32-bit BLAKE2s, 64-bit BLAKE2b, the
parallel variants BLAKE2sp and BLAKE2bp, and a framework for tree modes. Four
years later, the BLAKE2X standard added extensible output modes. None of these
are compatible with each other. Users have had to choose between optimizing for
32-bit or 64-bit platforms, and between short or long messages.

BLAKE3 eliminates these tradeoffs, by providing a single variant that is
efficient in all of those use cases. Internally, BLAKE3 divides its input into
2 KiB chunks and arranges those chunks as the leaves of a binary Merkle tree.
This avoids introducing any overhead for short messages, and it allows more and
more parallelism as messages get longer. That parallelism means that BLAKE3 can
use a compression function that is efficient on small architectures, without
compromising peak throughput on desktop and server systems with SIMD support.

The BLAKE3 compression function is based on BLAKE2s. The round function is
unchanged, and BLAKE3 directly benefits from the cryptanalysis done on BLAKE2.
In hindsight, we believe that the original choice of 10 rounds for BLAKE2s was
overly conservative, and BLAKE3 reduces the number of rounds to 7. BLAKE3 also
changes the setup and finalization steps of the compression function to
accomodate the internal tree mode, reduce the cost of keying, and support
extensible output.
