...
implementation uses 2-way row-based parallelism at 2~KiB, 4-way word-based
...
parallel. BLAKE3 and KangarooTwelve are very close in peak throughput, but
...
other two described above. Entire 4-word rows can be stored together within a
vector, as in the first approach, but larger vectors can be used to store
multiple such rows, each belonging to a different state matrix. For example, a
256-bit vector can store two full rows. Diagonalization and undiagonalization
steps are still needed in this arrangement, so the second approach is
preferable where possible. However, on platforms like x86 that do not support
64-bit vectors, this approach is the only way to implement 2-way parallelism
for 2-chunk inputs. The performance of this approach is visible at 2~KiB in
Figure~\ref{fig:avx512}.
